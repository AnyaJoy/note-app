...
export default function NoteList(props) {
    ...
    const handleDelete = (e) => {
        ...
        if (result) {
            let newNoteData = noteData.filter((item) => {return item.id !== e.target.value});
    ...
    function handleTitleInput(e) {setTitleInput(e.target.value)}
    function handleNoteInput(e) {setNoteInput(e.target.value)}

    //enabling the edit button
    useEffect(() => {
        if (noteModal !== "" ) {setEditDisabled(false)} else {setEditDisabled(true)}
        if (noteInput !== "" ) {setEditDisabled(false)} else {setEditDisabled(true)}
        if (titleInput == titleModal) {setEditDisabled(false)} else {setEditDisabled(true)}
    }, [titleInput])

    useEffect(() => {
        if (noteInput !== "" && noteInput !== noteModal) {setEditDisabled(true)} else {setEditDisabled(false)}
    }, [noteInput])
    ...
    function handleEdit(e) {
        ...
        //updating the object
        let updatedObject = {id: oldObject.id, note: oldObject.note, title: oldObject.title, date: formattedDate}
            ...
            return (
                <div key={item.id} className="note-wrapper">
                    ...
                    <div className="date-delete-wrapper">
                        ...
                        <button onClick={handleDelete} className={`delete-button-${!disabled}`} disabled={disabled} value={item.id}>Delete</button>
                    </div>

                    <div className="modal">
                        <button className="modal-button" value={item.id} onClick={openModal}></button>
                        <Modal
                            isOpen={modalIsOpen}
                        ...
                        >
                            ...
                            <div><br></br></div>    
                            <textarea onChange={handleTitleInput} className="title-modal">{titleModal}</textarea>
                            <textarea onChange={handleNoteInput} className="note-modal">{noteModal}</textarea>
                            <div className="date-delete-wrapper-modal">
                                <div className="date-modal">{dateModal}</div>
                                <button onClick={handleEdit} value={idModal} disabled={!editDisabled}className={`edit-button-modal-${editDisabled}`}>Edit</button>
                            </div>
                        </Modal>
                    </div>
                </div>
